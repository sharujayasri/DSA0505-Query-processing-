...
# Convert 'OrderDate' column to datetime format with specified format
sales_data['OrderDate'] = pd.to_datetime(sales_data['OrderDate'], format='%m-%d-%y')

# Create a pivot table for total sale amount region-wise, manager-wise, and salesperson-wise
pivot_table_total_sale = pd.pivot_table(sales_data, values='Sale_amt', index=['Region', 'Manager', 'SalesMan'], aggfunc='sum')

# Display the result
print("Total Sale Amount Region-wise, Manager-wise, and Salesperson-wise:")
print(pivot_table_total_sale)
